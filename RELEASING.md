...
1. Create or reuse a crates.io API token with publish access for `use-units`.
...
After the first manual crates.io release for `use-units` exists, the repository
...
- Purpose: confirms `use-units` already exists on crates.io, then runs
  `release-plz release`
...
```sh
cargo publish -p use-units --dry-run
```
